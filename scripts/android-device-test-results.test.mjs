...
import { ANDROID_DEVICE_TEST_COUNT as count, verifyAndroidDeviceTestResults } from './android-device-test-results.mjs'
...
test('accepts exactly the required passing tests bound to the requested serial', async (context) => {
  const root = await fixture(`tests="${count}" failures="0" errors="0" skipped="0"`)
...
test('rejects failures, skips, wrong totals, and results from another serial', async (context) => {
  const failed = await fixture(`tests="${count}" failures="1" errors="0" skipped="0"`)
  const skipped = await fixture(`tests="${count}" failures="0" errors="0" skipped="1"`)
  const short = await fixture(`tests="${count - 1}" failures="0" errors="0" skipped="0"`)
  const wrongDevice = await fixture(`tests="${count}" failures="0" errors="0" skipped="0"`, 'device-other')
  context.after(() => Promise.all([failed, skipped, short, wrongDevice].map((root) => fs.rm(root, { recursive: true, force: true }))))
  assert.throws(() => verifyAndroidDeviceTestResults(failed, 'device-123'), new RegExp(`实际 tests=${count}, failures=1`))
  assert.throws(() => verifyAndroidDeviceTestResults(skipped, 'device-123'), /skipped=1/)
  assert.throws(() => verifyAndroidDeviceTestResults(short, 'device-123'), new RegExp(`实际 tests=${count - 1}`))
...
test('rejects absent or malformed result evidence', async (context) => {
  ...
  const malformed = await fixture(`tests="${count}" failures="0" errors="0"`)
